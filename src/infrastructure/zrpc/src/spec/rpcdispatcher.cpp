...
void ZRpcDispatcher::dispatch(AbstractData *data, TcpConnection *conn) {
    ...
    DLOG << "begin to dispatch client request, msgno=" << tmp->msg_req << std::endl;
    ...
    DLOG << reply_pk.msg_req << "|response.name = " << response->GetDescriptor()->full_name() << std::endl;
...
bool ZRpcDispatcher::parseServiceFullName(const std::string &full_name,
                                          std::string &service_name,
                                          std::string &method_name) {
    if (full_name.empty()) {
...
void ZRpcDispatcher::registerService(service_ptr service) {
